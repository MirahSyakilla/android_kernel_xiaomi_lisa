...
#define trace_hyp_core_ctl_enable(enable) do { } while (0)
#define trace_hyp_core_ctl_status(hcd, msg) do { } while (0)
...
static int hyp_core_ctl_hp_state = CPUHP_INVALID;
static cpumask_t hyp_core_ctl_thermal_mask;

static inline const cpumask_t *hyp_core_ctl_thermal_cpumask(void)
{
	return &hyp_core_ctl_thermal_mask;
}

static inline int hyp_sched_isolate_cpu(int cpu)
{
	return cpu_down(cpu);
}

static inline int hyp_sched_unisolate_cpu(int cpu)
{
	return cpu_up(cpu);
}

static inline int hyp_sched_unisolate_cpu_unlocked(int cpu)
{
	return cpu_up(cpu);
}
...
static inline void hyp_core_ctl_print_status(char *msg)
{
	...
	pr_debug("%s: reserve=%*pbl reserved=%*pbl our_isolated=%*pbl online=%*pbl isolated=%*pbl thermal=%*pbl\n",
		...
		cpumask_pr_args(&the_hcd->our_isolated_cpus),
		cpumask_pr_args(hyp_core_ctl_thermal_cpumask()));
}
...
static void hyp_core_ctl_undo_reservation(struct hyp_core_ctl_data *hcd)
{
	...
	for_each_cpu(cpu, &hcd->our_isolated_cpus) {
		ret = hyp_sched_unisolate_cpu(cpu);
		if (ret < 0) {
...
static void hyp_core_ctl_do_reservation(struct hyp_core_ctl_data *hcd)
{
	...
	const cpumask_t *thermal_cpus = hyp_core_ctl_thermal_cpumask();
	struct freq_qos_request *qos_req;
...
static struct notifier_block hyp_core_ctl_nb __maybe_unused = {
...
static int hyp_core_ctl_hp_offline(unsigned int cpu)
{
	...
	if (cpumask_test_and_clear_cpu(cpu, &the_hcd->our_isolated_cpus)) {
		hyp_sched_unisolate_cpu_unlocked(cpu);
		if (freq_qos_init_done) {
			qos_req = &per_cpu(qos_min_req, cpu);
			ret = freq_qos_update_request(qos_req,
					FREQ_QOS_MIN_DEFAULT_VALUE);
...
static int hyp_core_ctl_probe(struct platform_device *pdev)
{
	...
	hyp_core_ctl_hp_state = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN,
						"qcom/hyp_core_ctl:online",
						hyp_core_ctl_hp_online,
						hyp_core_ctl_hp_offline);
	if (hyp_core_ctl_hp_state < 0) {
		ret = hyp_core_ctl_hp_state;
		pr_err("Failed to register cpuhp state. ret=%d\n", ret);
		goto remove_sysfs;
	}

	
	/* thermal notifier API not available on this tree */
...
remove_sysfs:
	sysfs_remove_group(&cpu_subsys.dev_root->kobj, &hyp_core_ctl_attr_group);
free_hcd:
	kfree(hcd);
