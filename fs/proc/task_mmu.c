...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (file) {
		...
		if (unlikely(test_bit(AS_FLAGS_SUS_MAP, &inode->i_mapping->flags) && susfs_is_current_proc_umounted()))
		{
			...
			seq_putc(m, '-');
			seq_putc(m, 'p');
			seq_put_hex_ll(m, " ", ((loff_t)vma->vm_pgoff) << PAGE_SHIFT, 8);
			seq_put_hex_ll(m, " ", MAJOR(inode->i_sb->s_dev), 2);
			seq_put_hex_ll(m, ":", MINOR(inode->i_sb->s_dev), 2);
			seq_put_decimal_ull(m, " ", inode->i_ino);
			seq_putc(m, ' ');
			goto done;
		}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
	if (vma->vm_file &&
		unlikely(test_bit(AS_FLAGS_SUS_MAP, &file_inode(vma->vm_file)->i_mapping->flags) && susfs_is_current_proc_umounted()))
	{
		show_map_vma(m, vma);
		SEQ_PUT_DEC("Size:           ", vma->vm_end - vma->vm_start);
		SEQ_PUT_DEC(" kB\nKernelPageSize: ", vma_kernel_pagesize(vma));
