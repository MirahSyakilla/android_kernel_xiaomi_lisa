...
{
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	// Skip freeing if it's a fake/backup mnt_id
				...
				void *data)
{
	...
	if (susfs_is_current_ksu_domain()) {
		mnt = &susfs_alloc_sus_vfsmnt(name ?: "none")->mnt;
		if (!mnt)
			goto out_free_context;
		goto bypass_fc_mount;
	}
#endif
...
out:
	...
	if (!susfs_is_boot_completed_triggered &&
		...
		// We spoof the destination path as dir_name
		char *pathname = kmalloc(PAGE_SIZE, GFP_KERNEL);
		if (pathname) {
			char *dpath = d_path(new_path, pathname, PAGE_SIZE);
			if (!IS_ERR(dpath)) {
				susfs_auto_add_sus_ksu_default_mount(dpath);
			}
			kfree(pathname);
		}
	}
...
{
	struct mnt_namespace *mnt_ns = current->nsproxy ? current->nsproxy->mnt_ns : NULL;
	struct mount *mnt;
	int first_mnt_id;
...
bool susfs_is_mnt_devname_ksu(struct path *path)
{
	...
	if (!path || !path->mnt)
		return false;
	mnt = real_mount(path->mnt);
	return mnt->mnt_devname && strcmp(mnt->mnt_devname, "KSU") == 0;
}
#endif