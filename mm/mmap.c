...
{
	while (find_vma_intersection(mm, start, start + len)) {
		if (do_munmap(mm, start, len, uf))
			return -ENOMEM;
	}

	find_vma_prev(mm, start, pprev);
	return 0;
}
 ...
 * mm's list and maple tree.  It has already been inserted into the interval tree.
 */
