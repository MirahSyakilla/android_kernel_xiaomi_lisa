    ...
    def __get_number_or_prop(arg):
        ...
        if arg[0] == '/':
            ...
            if arg.startswith('0x') or arg.startswith('0X'):
                base = 16
            elif arg.startswith('0o'): # Python 3 uses '0o' for octal
                base = 8
            elif arg.startswith('0') and len(arg) > 1 and arg[1:].isdigit(): # Handle legacy '0' prefix for octal
                base = 8
            return int(arg, base)
...
class Dtbo(object):
    ...
    def _read_dt_entries_from_metadata(self):
        ...
        offset_bytes = self.dt_entries_offset
        params = {}
        params['dt_file'] = None
        for i in range(0, self.dt_entry_count):
            entry_data = self.__metadata[offset_bytes:offset_bytes + self._DT_ENTRY_HEADER_SIZE]
            dt_table_entry = struct.unpack('>8I', entry_data)
            ...
            offset_bytes += self._DT_ENTRY_HEADER_SIZE
    ...
    def _read_dtbo_image(self):
        ...
        self.__file.seek(0)
        self.__metadata = self.__file.read(self.__metadata_size)
    ...
    def __str__(self):
        ...
        for key in _keys:
            ...
            if key == 'magic':
                sb.append('{key:>20} = {value:08x}'.format(key=key, value=value))
            else:
                sb.append('{key:>20} = {value:d}'.format(key=key, value=value))
        count = 0
        for dt_entry in self.__dt_entries:
...
def dump_dtbo_image(fin, argv):
    ...
    # Encode the string output to bytes before writing
    args.outfile.write((str(dtbo) + '\n').encode('utf-8'))
    if args.outfile is not stdout.buffer:
        args.outfile.close()
...
def print_usage(cmd, _):
    ...
    if not found and cmd != 'all':
        print(f'Unsupported help command: {cmd}\n')
        print_default_usage(prog_name)
...
def main():
    ...
    # Use parse_known_args() to separate the subcommand's arguments
    # from the rest of the command line arguments.
    args, remaining_args = parser.parse_known_args()

    if not args.command:
        print_default_usage(os.path.basename(__file__))
        return

    # Call the function set by set_defaults
    args.func(args.argfile, remaining_args)
